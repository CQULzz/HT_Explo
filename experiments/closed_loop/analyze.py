...
def main():
 ...
 lines=['# 山地真实 HT 闭环验证','','CMU 仿真；在线雷达输入真实八方向学生模型，两组均运行推理。配置与源码差异见 protocol.json。','','|组别|重复|新增地形体素|总覆盖率|轨迹 m|>15° 地形路径 m|结束距起点 m|真正完成|完成秒|降级秒|结束执行状态|','|---|---:|---:|---:|---:|---:|---:|---|---:|---:|---|']
 for r in rows:lines.append(f'|{r["condition"]}|{r["rep"]}|{r["new_surface_voxels"]}|{r["surface_coverage_pct"]:.2f}%|{r["distance_m"]:.2f}|{r["steep_distance_m"]:.2f}|{r["final_home_distance_m"]:.2f}|{r["mission_completed"]}|{r["mission_completion_time_s"]}|{r["fallback_s"]:.1f}|{r["final_execution_status"]}|')
