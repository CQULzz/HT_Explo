...
public:
  ...
  std::shared_ptr<ht_cost_ns::HtCostMap> HT() const { return ht_map_; }
  double GetHTShortestPath(int start, int target, nav_msgs::msg::Path& path);
...
private:
  ...
  std::vector<std::vector<double>> candidate_ht_dist_;
  std::shared_ptr<ht_cost_ns::HtCostMap> ht_map_;
